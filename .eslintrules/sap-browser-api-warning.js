...
module.exports = function(context) {
	...
	var FORBIDDEN_DOM_ACCESS = ["getElementById", "getElementsByClassName",
		...
		FORBIDDEN_HISTROY_USAGES = [
            "go", "back", "forward"],
		FORBIDDEN_LOCATION_USAGES = ["href",
            "hash", "assign"];
	var FULL_BLACKLIST = FORBIDDEN_DOM_ACCESS.concat(FORBIDDEN_WINDOW_USAGES,
		FORBIDDEN_HISTROY_USAGES, FORBIDDEN_LOCATION_USAGES);

	var FORBIDDEN_DOCUMENT_OBJECT = [],
		FORBIDDEN_SCREEN_OBJECT = [],
		FORBIDDEN_BODY_OBJECT = [],
		FORBIDDEN_HISTORY_OBJECT = [],
		FORBIDDEN_LOCATION_OBJECT = [];
	...
	function processVariableDeclarator(node) {
		if (node.init) {
			if (node.init.type === "MemberExpression") {
				var firstElement = node.init.object.name,
			...
			} else if ((node.init.type === "Identifier") && (node.init.name === "document")) {
				FORBIDDEN_DOCUMENT_OBJECT.push(node.id.name);
			} else if ((node.init.type === "Identifier") && (node.init.name === "screen")) {
				FORBIDDEN_SCREEN_OBJECT.push(node.id.name);
			} else if ((node.init.type === "Identifier") && (node.init.name === "location")) {
				FORBIDDEN_LOCATION_OBJECT.push(node.id.name);
			} else if ((node.init.type === "Identifier") && (node.init.name === "history")) {
				FORBIDDEN_HISTORY_OBJECT.push(node.id.name);
			}
		}
	}
	...
	function processSetTimeOut(node) {
		if ((node.callee.type === "Identifier") && (node.callee.name === "setTimeout") && (node.arguments) && (arguments[0].arguments[1].value >
			0)) {
	...
	function buildCalleePath(memberExpressionNode, calleePath) {
		if (memberExpressionNode.object) {
			if (memberExpressionNode.object.type === "MemberExpression") {
				return buildCalleePath(memberExpressionNode.object, calleePath) + "." + memberExpressionNode.object.property.name;
	...
	function checkAssignmentAgainstOverride(node) {
		if (node.left.type === "MemberExpression") {
			...
			var firstElement = memberExpression.object.name,
				secondElement = memberExpression.property.name;
			if (firstElement + "." + secondElement === "window.location") {

				context.report(node, MESSAGE_LOCATION_OVERR);
			}
		} else if (((node.left.type === "Identifier") && (node.left.name === "location")) || ((node.left.type === "Identifier") && (contains(
		...
		"MemberExpression": function(node) {
			if ((node.parent.type === "CallExpression") && (!node.computed)) {
				...
				if ((typeof methodName === "string") && (contains(FULL_BLACKLIST, methodName))) {
					...
					if ((speciousObject === "document") && (contains(FORBIDDEN_DOM_ACCESS, methodName))) {
						context.report(node, MESSAGE_DOM_ACCESS);
					} else if ((speciousObject !== "document") && (contains(FORBIDDEN_DOCUMENT_OBJECT,
						speciousObject))) {
						context.report(node, MESSAGE_DOM_ACCESS);
					} else if ((speciousObject === "history") && (contains(FORBIDDEN_HISTROY_USAGES, methodName))) {
						context.report(node, MESSAGE_HISTROY_USAGES);
					} else if (contains(FORBIDDEN_HISTROY_USAGES, methodName) && (speciousObject !== "history") && (contains(FORBIDDEN_HISTORY_OBJECT,
						speciousObject))) {
						context.report(node, MESSAGE_HISTROY_USAGES);
					}

					if ((speciousObject === "window") && (contains(FORBIDDEN_WINDOW_USAGES, methodName)) && (methodName === "setTimeout")) {
						context.report(node, MESSAGE_TIMEOUT);
					}

					if ((speciousObject === "window") && (contains(FORBIDDEN_WINDOW_USAGES, methodName)) && (methodName === "getSelection")) {
						context.report(node, MESSAGE_GLOBAL_SELECTION);
					}

					if (((speciousObject === "location") && (contains(
						FORBIDDEN_LOCATION_USAGES, methodName))) || ((speciousObject !== "location") && (contains(
				...
				if ((calleePathNonCmpt === "window") && (node.property) && (contains(FORBIDDEN_WINDOW_USAGES,
					node.property.name))) {
					/*
					 * window.innerHeight = 16; for exp
					 */
					context.report(node, MESSAGE_WINDOW_USAGES);
				} else if ((calleePathNonCmpt === "window.screen") || (calleePathNonCmpt === "screen") || (contains(FORBIDDEN_SCREEN_OBJECT,
					calleePathNonCmpt))) {
					context.report(node, MESSAGE_WINDOW_USAGES);
				}

				if ((calleePathNonCmpt === "history") || (calleePathNonCmpt === "window.history")) {
					context.report(node, MESSAGE_HISTROY_USAGES);
				}

				if (((calleePathNonCmpt === "document.body") || (calleePathNonCmpt === "window.document.body")) && (speciousObjectNonCmpt === "body")) {
					context.report(node, MESSAGE_WINDOW_USAGES);
				} else if (((speciousObjectNonCmpt === "body") && (contains(
					FORBIDDEN_DOCUMENT_OBJECT, calleePathNonCmpt.slice(0,
						calleePathNonCmpt.lastIndexOf(".body"))))) || (contains(FORBIDDEN_BODY_OBJECT, calleePathNonCmpt))) {
					context.report(node, MESSAGE_WINDOW_USAGES);
				}

				if ((((calleePathNonCmpt === "window.location") || (calleePathNonCmpt === "location")) && (node.property) && (contains(
					FORBIDDEN_LOCATION_USAGES, node.property.name))) || (contains(FORBIDDEN_LOCATION_OBJECT,
					calleePathNonCmpt))) {
