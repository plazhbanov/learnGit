...
module.exports = function(context) {
	...
	var FORBIDDEN_DOCUMENT_OBJECT = [],
		...
		FORBIDDEN_WINDOW_OBJECT = [],
		FORBIDDEN_WINDOW_EVENT_OBJECT = [];
	...
	function processDocumentMessage(node, methodName) {
		...
		} else if ((contains(FORBIDDEN_DOCUMENT_USAGE, methodName)) && (node.parent.arguments.length !== 0) && (node.parent.arguments[0].value ===
			"insertBrOnReturn")) {
	...
	function processVariableDeclarator(node) {
		if (node.init) {
			if (node.init.type === "MemberExpression") {
				...
				FORBIDDEN_LOCATION_OBJECT.push(node.id.name);
			} else if ((node.init.type === "Identifier") && (node.init.name === "navigator")) {
				context.report(node, MESSAGE_FORBIDDEN_PROPRIETARY_BROWSER_API);
			} else if ((node.init.type === "Identifier") && (node.init.name === "window")) {
				context.report(node, MESSAGE_FORBIDDEN_PROPRIETARY_BROWSER_API);
			...
			if (node.parent.type === "CallExpression") {
				...
				if ((typeof methodName === "string") && (contains(FULL_BLACKLIST, methodName))) {
					...
					if ((speciousObject === "document")) {
						...
						speciousObject))) {
						processDocumentMessage(node, methodName);
					} else if ((speciousObject !== "location") && (contains(FORBIDDEN_LOCATION_OBJECT,
						speciousObject))) {
						context.report(node, MESSAGE_LOCATION_RELOAD);
					} else if ((speciousObject !== "window") && (contains(FORBIDDEN_WINDOW_OBJECT,
						speciousObject))) {
						context.report(node,
							MESSAGE_FORBIDDEN_PROPRIETARY_BROWSER_API);
					}
				...
				if (node.computed) {
					...
					calleePathCmpt += buildCalleePath(node.object,
						calleePathCmpt);
					var speciousObjectCmpt = isForbiddenObviousApi(calleePathCmpt),
						methodNameCmpt = node.object.property ? node.object.property.name : -1;

					if (contains(FORBIDDEN_DYNAMIC_STYLE_INSERTION,
						methodNameCmpt) && (speciousObjectCmpt === "document")) {
						/*
						...
						methodNameCmpt) && (speciousObjectCmpt !== "document") && (contains(FORBIDDEN_DOCUMENT_OBJECT,
						speciousObjectCmpt))) {
						/*
					...
					if ((calleePathNonCmpt === "navigator") || (calleePathNonCmpt === "window.navigator") || ((calleePathNonCmpt === "window") && (node.property) &&
						(node.property.name === "navigator")) && (node.parent.parent.type !== "CallExpression") && (node.child === "undefined")) {
						/*
					...
					if ((calleePathNonCmpt === "window") && (node.property) && (!contains(FORBIDDEN_GLOB_EVENT,
						node.property.name))) {
						/*
						 * window.onresize = 16; for exp
						 */
						processWindowMessage(node, node.property.name);
					} else if ((calleePathNonCmpt === "window") && (node.property) && (contains(FORBIDDEN_GLOB_EVENT,
						node.property.name)) && (node.parent.type === "AssignmentExpression") && (node.parent.left === node)) {
						context.report(node, MESSAGE_FORBIDDEN_GLOB_EVENT);
					}

					if ((node.property) && ((node.property.name === "returnValue") || (node.property.name === "cancelBubble")) && (node.parent.type ===
						"AssignmentExpression") && (node.parent.left === node)) {
					...
					} else if ((calleePathNonCmpt.slice(-11) === "styleSheets") && (speciousObjectNonCmpt !== "document") && (contains(
