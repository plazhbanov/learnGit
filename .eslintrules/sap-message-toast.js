...
module.exports = function(context) {
	...
	function endsWith(s, sub) {
		return typeof s === "string" && typeof sub === "string" && s.substring(s.length - sub.length, s.length) === sub;
	}
					...
					switch (name) {
						case "duration":
							if (isInteger(value) && value < DURATION_MIN) {
								context.report(node, "Value for " + name + " of " + CALLEE_NAME + " should be greater or equal to " + DURATION_MIN + "!");
							}
							break;
						case "width":
							if (getEMValue(value) > WIDTH_MAX) {
								context.report(node, "Value for " + name + " of " + CALLEE_NAME + " should be less or equal to " + WIDTH_MAX + "em" + "!");
							}
							break;
						case "my":
						case "at":
							if (typeof value === "string" && value !== POSITION_DEFAULT) {
								context.report(node, "Value for " + name + " of " + CALLEE_NAME + " should be " + POSITION_DEFAULT + "!");
							}
							break;
						default:
					}
